...
**void-backup** | cp, mkdir, xbps | Android (voidlinux chroot) | Usage: ` void-backup [OPTION] ` <p> Creates backup of XBPS package cache in external storage. Run `void-backup --help` for more details. |
**void-deploy** | cp, mkdir, xbps | Android (voidlinux chroot) | Usage: ` void-deploy [OPTION] ` <p> Post-install and post-mount setup tool for Voidlinux chroot running on Android. It was created for, and will only work for chroot images created using `void-install`. Run `void-deploy --help` for more details. |
