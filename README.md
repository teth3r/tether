...
**omaps-backup** | date, ls, mkdir, pm | Android (root) | Usage: ` omaps-backup [OPTION] ` <p> This tool Create and restore backups of organic maps .mwm files in external storage (SD card, USB OTG, etc), It will fallback to using internal storage if no external storage device is available. Run `omaps-backup --help` for more details. |
